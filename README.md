...
- radar/cv/sigint 에이전트는 아직 목(mock) 로직 — 실제로는 sitrep_fusion_agent의
...
1. 사용자 PC에서 `test_multi_agent_skeleton.py` 재실행해서 동일 결과 재확인
2. radar/cv/sigint 목(mock) 로직을 sitrep_fusion_agent의 실제 연동 코드로 교체할지,
   아니면 이 신규 프로젝트에서 다시 만들지 결정
3. NASA FIRMS 등 실제 공개 열이상 데이터로 합성 데이터를 대체할지 검토
4. coordinator에 LLM 기반 SITREP 생성 붙일지 결정
